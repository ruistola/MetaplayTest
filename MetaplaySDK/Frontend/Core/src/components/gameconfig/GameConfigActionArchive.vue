...
MIconButton(
  ...
  //- Archive button.
  fa-icon(v-if="!singleGameConfigWithoutContents?.isArchived" icon="box-archive" class="tw-w-4 tw-h-4 tw-relative -tw-bottom-[3px]")
...
MActionModal(
  ...
  div(class="tw-flex tw-justify-between")
    div(class="tw-font-semibold") Archive Game Config
    MInputSwitch(
  ...
  span.small.text-muted Archiving a game config will hide it from the list of available configs. An archived config can be unarchived at any time.
</template>
 ...
 */
watch(() => singleGameConfigWithoutContents.value, (value) => {
  if (value) {
 ...
 */
function onShow () {
  archiveState.value = singleGameConfigWithoutContents.value?.isArchived ?? false
}
...
async function onOk () {
  ...
  showSuccessToast(`Game config ${archiveState.value ? 'archived' : 'unarchived'}.`)
